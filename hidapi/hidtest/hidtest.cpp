...
int main(int argc, char* argv[])
{
	...
	prepared.Initialize();
	setter.ResetColors();
	setter.SetKeyColor("Tab",254,254,254);
	setter.PushColors();
	...
	*/
	...
	while (true)
	{
		...
		for (uint32_t i = 0; i < messages.size(); i++)
		{
			...
			{
				if (message.optionalKey == "F13") {
					setter.SetKeyColor("F11", message.optionalR, message.optionalG, message.optionalB);
					setter.SetKeyColor("F12", message.optionalR, message.optionalG, message.optionalB);
				}
				else
